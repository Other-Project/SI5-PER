...
def main():
    with Reader(bag_path) as reader:
        ...
        print(f"Connection found for /crazyflie/odom")
        ...
        for connection, timestamp, rawdata in reader.messages(connections=crazyflie_odom_connections):
            ...
            drone_positions.append([msg.pose.pose.position.x,
        ...
        for connection, timestamp, rawdata in reader.messages(connections=alphabot2_odom_connections):
            msg = typestore.deserialize_cdr(rawdata, connection.msgtype)
            platform_positions.append([msg.pose.pose.position.x,
                                    msg.pose.pose.position.y, 
                                    msg.pose.pose.position.z]
                                   )
    ...
    print(f"\n{'='*60}")
    print(f"Distance to the moving platform")
    print(f"{'='*60}")
